...
class CalculusBackground(Scene):
    def construct(self):
        ...
        intro_text_top = Text('Calculus').next_to([-1.5,9,0]).set_color(color_gradient([BLUE,YELLOW],2)).scale(3.4)
        ...
        limit = MathTex(r"\lim_{x \to a} f(x) = L").next_to(intro_text_top,DOWN,buff=2).set_color(color_gradient([BLUE,GREEN],180)).scale(2)
        derivative = MathTex(r"f'(x) = \lim_{\Delta x \to 0} \frac{f(x+\Delta x) - f(x)}{\Delta x}").next_to(limit,DOWN,buff=2).set_color(color_gradient([BLUE,YELLOW],180)).scale(1.3)
        integral = MathTex(r"\int_a^b f'(x) dx = f(b)- f(a)").next_to(derivative,DOWN,buff=2).set_color(color_gradient([BLUE,BLUE],2)).scale(1.4)
        ...
        self.add(intro_text_top)
        self.add(limit)
        self.add(derivative)
        self.add(integral)
        self.add(vg,riemann_rectangles)
